...
and exp = (* 一つ一つの命令に対応する式 (caml2html: sparcasm_exp) *)
  ...
  | Set of int
  (*中略*)
  | Restore of Id.t * Type.t (* スタック変数から値を復元 (caml2html: sparcasm_restore) *)
*)
...
let expand xts ini addf addi =
  classify(*xts内の各(x,t)に対して繰り返す*)
    xts
    ini
    (fun (offset, acc) x ->
      (*let offset = align offset in*)
      (offset + 4, addf x offset acc))(*浮動小数点はシングルワードに*)(*t(xの型)がfloatの時は、オフセットを4増やし、関数addfを使う*)
    (fun (offset, acc) x t ->
      (offset + 4, addi x t offset acc))(*t(xの型)がfloat以外の時は、オフセットを4増やし、関数addiを使う*)

let rec g env = function (*式の仮想マシンコード生成 (caml2html: virtual_g) *)
  | Closure.Unit -> Ans(Nop)
  | Closure.Int(i) -> Ans(Set(i))(*emit.mlでaddi命令に変換される*)
  | Closure.Float(d) ->
      let l =
        try
          (* すでに定数テーブルにあったら再利用 *)
      ...
      Let((x, Type.Int), SetL(l), Ans(LdDF(x, C(0))))    (*変数(レジスタ)にラベルを入れた後、ロード命令をする。これで定数テーブル上の浮動小数点数を引き出せる*)
  ...
  | Closure.Floor(x) -> Ans(FFloor(x))
  | Closure.Fsqrt(x) -> Ans(FSqrt(x))
  | Closure.Fabs(x) -> Ans(FAbs(x))
  | Closure.FtoI(x) -> Ans(Ftoi(x))
  | Closure.ItoF(x) -> Ans(Itof(x))
  | Closure.ReadInt(x) -> Ans(Read_int())
  | Closure.ReadFloat(x) -> Ans(Read_float())
  | Closure.PrintInt(x) -> Ans(Print_int(x))
  | Closure.PrintChar(x) -> Ans(Print_char(x))
  ...
  | Closure.Tuple(xs) -> (* 組の生成 (caml2html: virtual_tuple) *)
      ...
      let (offset, store) =
        expand
          ...
          (0, Ans(Mov(y)))
          (*初期の(オフセット,組生成後の式(ヒープ中の組の先頭のアドレスが入ったレジスタからのmov命令))*)
          (*let seq(e1, e2) = Let((Id.gentmp Type.Unit, Type.Unit), e1, e2)*)
         (* seqは shorthand of Let for unit *)
    ...
    (*以下は、組を生成して組の先頭のアドレスを返す命令列。*)
      Let((y, Type.Tuple(List.map (fun x -> M.find x env) xs)), Mov(reg_hp),(*現在のヒープポインタの値を組の先頭のアドレスとする*)
          Let((reg_hp, Type.Int), Add(reg_hp, C(offset)),(*ヒープポインタの値を組のサイズ分だけ増やす*)
              store))(*組の要素をストアした後、組の先頭のアドレスを返す*)(*組の先頭のアドレスを組そのものの値としている。組の読み出しはこのアドレスを介して行う。*)
  ...
  | Closure.Get(x, y) -> (* 配列の読み出し(caml2html: virtual_get) *)(* x[y] を取り出す。*)
      ...
      | Type.Array(Type.Float) ->
          Let((offset, Type.Int),SLL(y,C(2)),(*float:single*)(*アドレスx+4*yからロード*)
              Ans(LdDF(x, V(offset))))
      | Type.Array(_) ->
          Let((offset, Type.Int),SLL(y,C(2)),(*アドレスx+4*yからロード*)
              Ans(Ld(x, V(offset))))
  ...
  | Closure.Put(x, y, z) ->(*配列への書き込み*)
      ...
      | Type.Array(Type.Float) ->
          Let((offset, Type.Int),SLL(y,C(2)),(*floatはsingle*)(*アドレスx+4*yにzを書き込む*)
              Ans(StDF(z, x, V(offset))))
      | Type.Array(_) ->
          Let((offset, Type.Int),SLL(y,C(2)),(*アドレスx+4*yにzを書き込む*)
              Ans(St(z, x, V(offset))))
  ...
  let (offset, load) =
    expand
      ...
      (4, g (M.add x t (M.add_list yts (M.add_list zts M.empty))) e)
       (*自由変数と引数の変数と関数のラベルを入れた環境(名前→型)で、関数の本体の式を仮想マシンコードに変換したものが初期の命令列である。初期のoffsetは4とする。*)
       (*let fletd(x, e1, e2) = Let((x, Type.Float), e1, e2) *)
      (fun z offset load -> fletd(z, LdDF(x, C(offset)), load))
      (*命令列の先頭に、クロージャから自由変数の値をロードする命令を追加する*)
