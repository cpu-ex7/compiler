...
let rec target' src (dest, t) = function
  ...
  | CallDir(_, ys, zs) ->
      true, (target_args src regs 0 ys @
             target_args src fregs 0 zs)
  | _ -> false, []
and target src dest = function (* register targeting, 変数srcが以降の命令列で特定のレジスタで使われるか調べる、使われたレジスタのリストを返す (caml2html: regalloc_target) *)
...
and g'_and_restore dest cont regenv exp = (* 使用される変数をスタックからレジスタへRestore(caml2html: regalloc_unspill) *)
  ...
  with NoReg(x, t) ->
    ((* Format.eprintf "restoring %s@." x; *)
     g dest cont regenv (Let((x, t), Restore(x,t), Ans(exp))))
and g' dest cont regenv = function (* ��̿���Υ쥸������������ (caml2html: regalloc_gprime) *)
  | Nop | Set _ | SetL _ | Comment _ | Restore _ | Read_int _ | Read_float _ as exp -> (Ans(exp), regenv)
  ...
  | FFloor(x) -> (Ans(FFloor(find x Type.Float regenv)), regenv)
